...
**1. New field on each finance entry** (`purchase.finance[i]`): `address` (string, the lender's full postal address). The current default shape (created by the "+ Add finance" button's handler) is `{name:"",amount:"",rate:"",dateLoaned:"",paybackDate:""}` — this change adds `address:""` to that same object literal, AND adds `address` to `ensurePurchase()`'s backfill list so existing entries (saved before this feature) get it defaulted to `""` too. Both call sites must be updated, not just the backfill.
...
**4. New printable view: the agreement itself.** A new `renderLoanAgreement(d, financeIndex)` function, routed via the existing `view` state mechanism the same way `renderSummary` is — but `go(v,id)` (the existing navigation helper) only carries one id, resolved via `curDeal()`/`currentId`, with no second slot for "which finance entry." This requires a new module-level state variable, `let currentFinanceIndex = null;`, declared alongside the existing `view`/`currentId`/`tab` state. The "Generate agreement" button sets `currentFinanceIndex = i` (the entry's index in `p.finance`) before navigating, e.g. `currentFinanceIndex=i; go("agreement", d.id);`. The router's dispatch (where `view==="summary"` currently calls `renderSummary(curDeal())`) gains a matching `else if(view==="agreement") renderLoanAgreement(curDeal(), currentFinanceIndex);` branch. This is the explicit mechanism to implement — don't invent a different one (e.g. don't try to thread the index through the deal id, don't add it as a third positional argument to `go()`, which would require changing every other existing call site of `go()`). Reachable only via the "Generate agreement" button (no direct URL/nav entry point needed, matching how `summary` is only reached via the "Share summary" button). Structurally mirrors the reviewed template:
...
- Numbered clauses 1–10, verbatim from the template, with the loan amount/dates/rate substituted into the same handful of spots the original template fills them in — including clause 2 (Purpose of Loan and Security: "fund property investment", and the Personal Guarantee tied to the named Guarantor) and clause 9 (Borrower's warranties) and clause 10 (Miscellaneous matters, including the notices sub-clause and the England & Wales governing-law sub-clause) — every sub-clause in the reviewed document, not a summarized subset
